...
class Classifier {
  ...
  Map<String, dynamic>? predict(imageLib.Image image) {
    ...
    // 예측 확률이 가장 높은 객체를 찾기 위한 변수들
    double maxScore = 0;
    Recognition? bestRecognition;

    // 각 인식 결과를 Recognition 객체로 생성하고 예측 확률이 가장 높은 객체를 찾습니다.
    for (int i = 0; i < resultsCount; i++) {
      var score = outputScores.getDoubleValue(i);
      var labelIndex = outputClasses.getIntValue(i) + labelOffset;
      var label = _labels?.elementAt(labelIndex);
      if (score > maxScore && score > THRESHOLD) {
        ...
        bestRecognition = Recognition(i, label, score, transformedRect);
      }
    }

    // 예측 확률이 가장 높은 객체만 recognitions 목록에 추가합니다.
